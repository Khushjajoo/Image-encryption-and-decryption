...
class des():
    ...
    def substitute(self, d_e):#Substitute bytes using SBOX
        ...
        while i < len(subblocks):
            list.append(self.compute_s_box(subblocks[i],i+1))
            i += 1
        result = []
        for i in list:
            for j in i:
    ...
    def compute_s_box(self,block,round):
        ...
        row = 0
        if(block[0] == 0 and block[5] == 1):
            row = 1
        elif(block[0] == 1 and block[5] == 0):
            row = 2
        elif(block[0] == 1 and block[5] == 1):
            row = 3
        t_block = block[1:5]
        bin = binvalue(S_BOX[round-1][row][int(''.join(str(e) for e in t_block), 2)], 4)
        return bin
